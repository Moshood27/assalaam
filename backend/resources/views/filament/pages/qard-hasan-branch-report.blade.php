            ...
            <div class="rounded-xl border border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900 shadow-sm overflow-hidden break-inside-avoid mb-8">
                <div class="bg-gray-50 dark:bg-gray-800/60 px-5 py-3 border-b border-gray-200 dark:border-gray-800 flex justify-between items-center">
                    ...
                    <div class="text-sm font-medium text-gray-500 dark:text-gray-400">
                        Outstanding: <span class="text-primary-600 dark:text-primary-400 font-bold">â‚¦{{ number_format($branch['total_outstanding'], 2) }}</span>
                    </div>
                </div>
                    ...
                    <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-800 text-sm">
                        ...
                        <tbody class="divide-y divide-gray-100 dark:divide-gray-800">
                            @foreach($branch['loans'] as $loan)
                                <tr class="hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors">
                                    ...
                                    <td class="px-5 py-3 text-right tabular-nums text-gray-900 dark:text-gray-100">â‚¦{{ number_format($loan['principal'], 2) }}</td>
                                    <td class="px-5 py-3 text-right tabular-nums text-gray-900 dark:text-gray-100">â‚¦{{ number_format($loan['paid'], 2) }}</td>
                                    <td class="px-5 py-3 text-right tabular-nums text-red-600 dark:text-red-400 font-bold">â‚¦{{ number_format($loan['overdue'] ?? 0, 2) }}</td>
                                    <td class="px-5 py-3 text-right tabular-nums font-semibold text-primary-600 dark:text-primary-400">â‚¦{{ number_format($loan['outstanding'], 2) }}</td>
                        ...
                        <tfoot class="bg-gray-50/50 dark:bg-gray-800/40 font-bold border-t border-gray-200 dark:border-gray-800">
                            <tr>
                                ...
                                <td class="px-5 py-4 text-right tabular-nums text-gray-900 dark:text-gray-100">â‚¦{{ number_format($branch['total_principal'], 2) }}</td>
                                <td class="px-5 py-4 text-right tabular-nums text-gray-900 dark:text-gray-100">â‚¦{{ number_format($branch['total_paid'], 2) }}</td>
                                <td class="px-5 py-4 text-right tabular-nums text-red-600 dark:text-red-400">â‚¦{{ number_format($branch['total_overdue'] ?? 0, 2) }}</td>
                                <td class="px-5 py-4 text-right tabular-nums text-primary-600 dark:text-primary-400">â‚¦{{ number_format($branch['total_outstanding'], 2) }}</td>
                                <td colspan="2"></td>
                            </tr>
                        </tfoot>
                    </table>
            ...
            <div class="rounded-xl border-2 border-primary-500 dark:border-primary-400 bg-primary-50/50 dark:bg-primary-900/20 p-6 break-inside-avoid">
                ...
                <div class="grid grid-cols-1 md:grid-cols-5 gap-4">
                    ...
                    <div class="p-4 bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700">
                        ...
                        <div class="text-2xl font-bold text-gray-900 dark:text-gray-100">â‚¦{{ number_format($report['grand_total_principal'], 2) }}</div>
                    </div>
                    <div class="p-4 bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700">
                        <div class="text-xs text-gray-500 dark:text-gray-400 uppercase font-semibold">Total Paid</div>
                        <div class="text-2xl font-bold text-gray-900 dark:text-gray-100">â‚¦{{ number_format($report['grand_total_paid'], 2) }}</div>
                    </div>
                    <div class="p-4 bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-red-200 dark:border-red-800 ring-2 ring-red-500/10">
                        ...
                        <div class="text-2xl font-black text-red-700 dark:text-red-300">â‚¦{{ number_format($report['grand_total_overdue'] ?? 0, 2) }}</div>
                    </div>
                    <div class="p-4 bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-primary-200 dark:border-primary-800 ring-2 ring-primary-500/20">
                        ...
                        <div class="text-2xl font-black text-primary-700 dark:text-primary-300">â‚¦{{ number_format($report['grand_total_outstanding'], 2) }}</div>
                    </div>
                </div>
            </div>
        @endif
