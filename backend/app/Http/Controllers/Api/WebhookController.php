...
class WebhookController extends Controller
{
    public function handlePaystack(Request $request)
    {
        ...
        // Handle Dedicated Virtual Account Webhooks
        if (in_array($event, ['dedicatedaccount.assign.success', 'dedicatedaccount.assign.failed'])) {
            $customerCode = $data['customer']['customer_code'] ?? null;
            $user = null;
            if ($customerCode) {
                $user = User::whereHas('virtualAccount', fn($q) => $q->where('paystack_customer_code', $customerCode))->first();
                ...
                if ($event === 'dedicatedaccount.assign.success') {
                    ...
                    if ($acc) {
                        $user->virtualAccount()->updateOrCreate([], [
                            ...
                            'dva_bank_name'      => $acc['bank']['name'],
                        ]);
                        Log::info('Paystack DVA assigned via webhook', ['user_id' => $user->id, 'account_number' => $acc['account_number']]);

                        $user->notifyMember('Virtual Account Ready', "Your dedicated virtual account ({$acc['bank']['name']}) is now ready for use.", [
                            'type' => 'dva_ready',
        ...
        // Handle Customer Identification Webhooks
        if (in_array($event, ['customeridentification.success', 'customeridentification.failed'])) {
            $customerCode = $data['customer_code'] ?? ($data['customer']['customer_code'] ?? null);
            $user = null;
            if ($customerCode) {
                $user = User::whereHas('virtualAccount', fn($q) => $q->where('paystack_customer_code', $customerCode))->first();
            }
