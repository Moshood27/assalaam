<template>
  <div class="space-y-4">
    ...
    <div class="bg-white rounded-3xl shadow-sm border border-slate-100 p-5">
      ...
      <div class="space-y-3">
        <a v-for="(p, i) in phones" :key="i" :href="wa(p.number)" target="_blank" class="flex items-center justify-between p-4 rounded-2xl border hover:bg-blue-50 transition-colors group">
          ...
          <span class="text-blue-700 font-bold">Open</span>
        </a>
      </div>
    </div>
    ...
    <!-- Email Contact -->
    <div class="bg-white rounded-3xl shadow-sm border border-slate-100 p-5">
      <p class="text-[10px] text-slate-400 font-black uppercase tracking-widest mb-3">Email Support</p>
      <div class="flex items-center justify-between">
        <p class="text-slate-700 text-sm">
          Reach us at
          <a :href="`mailto:${email}`" class="text-blue-700 font-bold hover:underline ml-1">{{ email }}</a>
        </p>
...
const email = 'assalaamosogbo@gmail.com'
