      ...
      <section class="bg-white rounded-[2.5rem] p-8 shadow-sm border border-slate-100">
        <div class="flex items-center justify-between mb-8">
          <div>
            ...
            <p class="text-[10px] font-black text-blue-600 uppercase tracking-widest">Track your coop store orders</p>
          </div>
        ...
        <div v-else>
          ...
          <div v-else class="space-y-4">
            <div v-for="o in items" :key="o.id" 
              class="group relative bg-slate-50 hover:bg-white p-5 rounded-[2rem] border border-transparent hover:border-slate-100 hover:shadow-xl hover:shadow-blue-900/5 transition-all duration-300 cursor-pointer overflow-hidden" 
              ...
              <div class="flex items-center justify-between pt-4 border-t border-slate-200/50">
                <div class="flex -space-x-2">
                   ...
                   <div v-if="(o.items || []).length > 3" class="w-8 h-8 rounded-full border-2 border-white bg-blue-600 text-white flex items-center justify-center text-[8px] font-black shadow-sm">+{{ o.items.length - 3 }}</div>
                </div>
                <div class="flex items-center gap-2">
                   ...
                   <span class="i-mdi-chevron-right text-xl text-slate-300 group-hover:text-blue-500 transition-colors"></span>
                </div>
              </div>
            </div>
          </div>
