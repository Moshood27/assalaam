    ...
    <div class="container-app py-4">
      ...
      <div v-else class="space-y-4">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div v-if="eligibility.assalaam_score !== undefined" class="card p-4 flex items-center gap-4">
                    ...
                    <div>
                      ...
                      <p class="text-lg font-black text-slate-800">
                        {{ Math.round((eligibility.assalaam_score || 0) * 10) / 10 }}
                  ...
                  <div v-if="eligibility.meeting_attendance_count !== undefined" class="card p-4 flex items-center gap-4">
                    ...
                    <div>
                      ...
                      <p class="text-lg font-black" :class="eligibility.meeting_attendance_count >= eligibility.required_loan_meetings ? 'text-blue-600' : 'text-amber-600'">
                ...
                <div class="card overflow-hidden">
                  <div class="p-4 bg-slate-50 border-b border-slate-100 flex items-center justify-between">
                    ...
                    <div class="flex gap-2">
                      <router-link to="/loans/analysis" class="text-[10px] font-black text-blue-600 uppercase tracking-widest bg-blue-50 px-3 py-1 rounded-full border border-blue-100 hover:bg-blue-100 transition-colors">Analysis</router-link>
                  ...
                  <div class="p-6 space-y-6">
                        ...
                        <div v-for="i in 3" :key="i" class="relative group">
                          <div class="relative">
                            <input 
                              ...
                              type="text" 
                              class="input h-12 pl-12 pr-10 border-slate-200 focus:border-blue-500 transition-all" 
                    ...
                    <div class="pt-4 border-t border-slate-100">
                      ...
                      <button class="w-full bg-blue-600 hover:bg-blue-700 text-white h-14 rounded-2xl font-black uppercase tracking-widest shadow-lg shadow-blue-200 transition-all disabled:opacity-50 disabled:shadow-none flex items-center justify-center gap-2" :disabled="creating" @click="createLoan">
              ...
              <div class="card p-8 text-center space-y-4" v-else>
                ...
                <div class="pt-2">
                  <router-link to="/loans/analysis" class="text-[11px] font-black text-blue-600 uppercase tracking-widest bg-blue-50 px-6 py-2 rounded-full border border-blue-100 hover:bg-blue-100 transition-colors">
                    View My Analysis
                  </router-link>
                </div>
              </div>
                    ...
                    <li v-for="req in guarantorRequests" :key="req.id" class="p-5 hover:bg-slate-50/50 transition-colors flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                      ...
                      <div class="flex items-center justify-between sm:justify-end gap-4 border-t sm:border-t-0 pt-4 sm:pt-0">
                        <div class="text-right sm:block hidden">
                          ...
                          <span :class="req.guarantor_status === 'accepted' ? 'bg-blue-100 text-blue-700' : (req.guarantor_status === 'declined' ? 'bg-slate-100 text-slate-500' : 'bg-amber-100 text-amber-700')" class="px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-wider">
                            {{ req.guarantor_status }}
                          </span>
                        </div>

                        <div class="flex items-center gap-2" v-if="req.guarantor_status === 'pending'">
                          <button class="h-10 px-4 bg-blue-600 hover:bg-blue-700 text-white rounded-xl text-[11px] font-black uppercase tracking-widest shadow-lg shadow-blue-100 transition-all flex items-center justify-center min-w-[80px]" :disabled="!!grAction[req.id]" @click="acceptGuarantor(req)">
                      ...
                      <p v-if="grMsg[req.id]" class="w-full text-[11px] text-blue-700 font-bold bg-blue-50 p-2 rounded-lg text-center">{{ grMsg[req.id] }}</p>
                    </li>
                  </ul>
                </div>
              </div>
          ...
          <div class="p-6 space-y-6">
            ...
            <div>
              ...
              <div class="h-3 bg-slate-100 rounded-full overflow-hidden flex shadow-inner">
                <div class="h-full bg-blue-500 rounded-full transition-all duration-1000 relative" :style="{ width: (loan.paid_amount / loan.principal_amount * 100) + '%' }">
            ...
            <div class="pt-6 border-t border-slate-100 space-y-4" v-if="!loan.is_completed && ['active', 'defaulted'].includes(loan.status)">
              <div class="flex items-center justify-between">
                ...
                <span class="text-[10px] text-blue-600 font-bold">Auto-source enabled</span>
              </div>
            ...
            <p v-if="payMsg[loan.id]" class="text-xs text-blue-700">{{ payMsg[loan.id] }}</p>
...
const eligibility = ref({ savings: 0, shares: 0, base: 0, eligibility: 0, eligibility_adjusted: 0, months_in_system: 0, is_first_loan: true, can_request: false, reason: '', assalaam_score: 0, score_bonus_pct: 0, band: '', instant_approval: false, required_guarantors: 2 })
...
const getStatusBadgeClass = (loan) => {
  if (loan.is_completed) return 'bg-blue-100 text-blue-700'
  if (loan.overdue_amount > 0) return 'bg-rose-100 text-rose-700'
  if (loan.status === 'active') return 'bg-blue-100 text-blue-700'
