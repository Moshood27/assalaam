      ...
      <section v-if="tab === 'vendors'" class="space-y-4 animate-in fade-in duration-300">
        ...
        <div v-else>
          ...
          <div v-else class="space-y-3">
            <div v-for="v in items" :key="v.id" class="p-4 bg-white border rounded-xl shadow-sm space-y-3 hover:border-blue-200 transition-colors">
              ...
              <div class="pt-3 border-t flex flex-wrap gap-2">
                <button v-if="!v.is_approved" @click="approveVendor(v)" class="px-3 py-1.5 rounded-lg bg-blue-600 text-white text-xs font-bold shadow-sm hover:bg-blue-700 transition-colors">Approve</button>
          ...
          <div v-else class="space-y-3">
            <div v-for="s in items" :key="s.id" class="p-4 bg-white border rounded-xl shadow-sm space-y-3">
              ...
              <div v-if="s.status === 'pending'" class="pt-3 border-t flex gap-2">
                <button @click="approveSettlement(s)" class="flex-1 px-3 py-2 rounded-xl bg-blue-600 text-white text-xs font-bold shadow-md hover:bg-blue-700 transition-all active:scale-95">Mark Paid</button>
...
const statusClass = (status) => {
  if (status === 'paid') return 'bg-blue-50 text-blue-700 border-blue-100'
