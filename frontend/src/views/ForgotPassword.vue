    ...
    <div class="w-full max-w-md relative">
      ...
      <div class="card card-elevated relative overflow-hidden p-8 sm:p-10 bg-white/90 backdrop-blur-2xl border border-white/80 shadow-2xl rounded-[2.5rem]">
        ...
        <div class="flex flex-col items-center text-center mb-8">
          <div class="w-20 h-20 rounded-[1.75rem] bg-gradient-to-br from-blue-500 to-indigo-600 flex items-center justify-center text-white text-3xl shadow-xl shadow-blue-500/20 mb-4 transform hover:scale-105 transition-all">
        ...
        <div class="space-y-6">
          ...
          <div v-if="step === 1" class="space-y-6">
            <div class="space-y-3">
              ...
              <div class="flex p-1 bg-slate-100 rounded-2xl">
                <button @click="channel = 'email'" type="button" :class="['flex-1 py-3 rounded-xl font-bold text-sm transition-all', channel==='email' ? 'bg-white text-blue-800 shadow-sm' : 'text-slate-500 hover:text-slate-700']">
                  Email
                </button>
                <button @click="channel = 'sms'" type="button" :class="['flex-1 py-3 rounded-xl font-bold text-sm transition-all', channel==='sms' ? 'bg-white text-blue-800 shadow-sm' : 'text-slate-500 hover:text-slate-700']">
                  SMS
                </button>
                <button @click="channel = 'push'" type="button" :class="['flex-1 py-3 rounded-xl font-bold text-sm transition-all', channel==='push' ? 'bg-white text-blue-800 shadow-sm' : 'text-slate-500 hover:text-slate-700']">
                  Push
                </button>
              </div>
            </div>
            ...
            <div class="text-center">
              <router-link class="text-blue-700 font-bold hover:text-blue-800 flex items-center justify-center gap-1" to="/login">
          ...
          <div v-else class="space-y-6">
            <div class="rounded-2xl border border-blue-100 bg-blue-50/40 p-5 text-sm">
              <div class="font-bold text-blue-800 mb-1">Secure code sent!</div>
              <div v-if="sentTo" class="text-blue-700 space-y-1">
              ...
              <div v-else class="text-blue-700">Check your {{ channel === 'email' ? 'email' : (channel === 'push' ? 'app' : 'SMS') }}. Code expires in 10 mins.</div>
            </div>
            ...
            <div class="text-center">
              <button @click="backToRequest" class="text-[11px] font-black text-blue-700 uppercase tracking-widest hover:underline">Resend or change method</button>
            </div>
          </div>
        </div>
      </div>
