...
```bash
docker exec -it islamcoop-app composer install --optimize-autoloader --no-dev
```
...
```bash
docker exec -it islamcoop-app php artisan migrate --force
```
...
```bash
docker exec -it islamcoop-app php artisan optimize
