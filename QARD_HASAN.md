...
- Members can request a loan; depending on their As-Salaam Score and policy rules, they may need 2–3 guarantors.
...
- As-Salaam Score limit boost (applies only after the first loan is completed):
  - Score ≥ 90 → +15%
  - Score ≥ 80 → +10%
  - Score ≥ 70 → +5%
- Guarantor requirement from As-Salaam Score:
...
- GET /api/loans/eligibility — show member eligibility, As-Salaam Score, required_guarantors, and boosted limits
...
- Guarantor requirements scale with As-Salaam Score; instant approval requires none and proceeds to immediate disbursement/activation.
...
- Instant approval test:
  - Temporarily set a high As-Salaam Score for the member (or use a fixture) to trigger 0 guarantors and instant activation/disbursement.
- Repayment (wallet):
