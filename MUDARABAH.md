# Mudarabah (Pooled Investment Projects) – Flow Guide
...
- The Cooperative may create projects (e.g., Poultry Farm, Real Estate). Members can buy “shares” in these projects by paying into a regular scheme and tagging that payment to a project.
...
- ProjectInvestment
  - Represents the investor’s “share” entry in the project, created from a successful, project-linked Contribution.
...
5. When each project-linked Contribution moves to success, the Contribution model’s boot hooks create a ProjectInvestment for that user and project (idempotent, one per contribution).
...
Notes:
- If a Paystack webhook arrives with no matching pending Contributions for the reference, it is treated as a wallet top-up (via dedicated virtual account or direct charge) and credited to the member’s wallet instead.
...
2. From /pay, member chooses “Pay from wallet” and enters their 4-digit Transaction PIN.
...
- Project-specific investments are not listed line-by-line in the Passbook, but the link exists via Contribution.project_id and dedicated endpoints:
  - GET /api/projects – list active projects for members
  - GET /api/projects/{id} – project details
  - GET /api/projects/{id}/investments – member’s investments in that project (includes total_invested)
...
## 8) Worked Example
1. Admin creates “Poultry Farm 2026” with management_fee_percent=10, active=true.
2. Member selects Scheme “Investments” and Project “Poultry Farm 2026”, amount ₦50,000, then completes Paystack checkout.
3. Webhook verifies the payment and marks the linked Contribution success.
4. The system auto-creates a ProjectInvestment for the member with amount ₦50,000 on Poultry Farm 2026; Passbook shows ₦50,000 under the chosen scheme for that month; Admin can see the investment under Project Investments.
...
- For full gateway flow (staging):
  ...
  - Confirm via provider’s test webhook that Contributions are marked success and investments are created.
...
- ProjectInvestment: The member’s recorded stake in a project (created from a successful, project-linked Contribution).
...
- Wallet: Member’s in-app balance used to allocate funds into schemes without external checkout.
...
- Add member-facing pages that summarize each project’s performance and the member’s realized/expected profit shares.
